...
void ULoadoutWidget::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	if (Character && Character->GetInventoryComponent())
	{
        ...
        for (int32 i = 0; i < 10; i++)
        {
            if (PerkWidgets[i])
            {
                PerkWidgets[i]->UpdateSlot(Perks.IsValidIndex(i) ? (AItemBase*)Perks[i] : nullptr, i);
            }
        }
		
		// Update Inventory Bag Slots
...
void ULoadoutWidget::OnCloseClicked()
{
    APlayerCharacter* Character = Cast<APlayerCharacter>(GetOwningPlayerPawn());
    if (Character)
    {
        Character->ToggleLoadout();
    }
    else
    {
        RemoveFromParent();
